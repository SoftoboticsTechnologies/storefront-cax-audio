...
Open [http://localhost:1001](http://localhost:1001) with your browser to see the result.
...
deployment remains a static export created by `npm run build`. To inspect that
exact export locally, run `npm run serve` (build and preview) or `npm run
preview` after an existing build.
